  ...
  compare () {
    return new Promise((resolve, reject) => {
      ...
      differentpoints.forEach((value) => {
          ...
          boxes.forEach((box, index) => {
              ...
              if ( Math.sqrt(dx*dx + dy*dy) < 75 ) {
                  if (value.y < box.start.y) boxes[index].start.y = value.y;
                  if (value.x > box.finish.x) boxes[index].finish.x = value.x;
                  if (value.y > box.finish.y) boxes[index].finish.y = value.y;
                  newbox = false;
              }
          });
          if (newbox) {
              boxes.push({ 'start': { 'x': value.x, 'y': value.y }, 'finish': { 'x': value.x, 'y': value.y } });
              newbox = false;
          } else {
              boxes = boxes;
          }
      ...
      boxes.forEach((value, index) => {
          boxes[index].finish.x += 10;
          boxes[index].finish.y += 10;
          boxes[index].start.x -= 10;
          boxes[index].start.y -= 10;
      })
      ...
      if (_this._options.renderComposition) {
          _this._compositionImage.render().then(function(image) {
              resolve({ differences: differences, percentageDifference: percentageDifference, threshold: threshold, passed: passed, compositionImage: image, boxes: boxes, width: width, height: height });
          });
      } else {
          resolve({ differences: differences, percentageDifference: percentageDifference, threshold: threshold, passed: passed, boxes: boxes, width: width, height: height  });
      }
    })
  }
